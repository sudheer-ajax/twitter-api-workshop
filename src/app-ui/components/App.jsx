...
class AppComponent extends React.Component {
        ...
        if (window.opener) {
            ...
            window.onmessage = async ({ data: { type, data } }) => {
                if (type === "authorized") {
                    ...
                    axios(getTweetsConfig.apiUrl, {
                        method: getTweetsConfig.method,
                        headers: {
    ...
    async handleLoginClick() {
        ...
        axios(requestTokenConfig.apiUrl, {
            method: requestTokenConfig.method,
            headers: {
                'Content-Type': 'application/json'
            },
            data: { source : "twitter"}
            ...
            if (response.data.oauth_callback_confirmed === "true") {
                ...
                this.handleError(
                    `Callback URL  is not confirmed. Please check that is whitelisted within the Twitter app settings.`
                );
            }
    ...
    handleSortTweets(e) {
        this.setState({
            ...this.state,
            tweets : this.state.tweets.sort((prev,next) => {
                if (prev[e.target.value] < next[e.target.value]) {
                    return -1;
                }
                if (prev[e.target.value] > next[e.target.value]) {
                    return 1;
    ...
    render() {
        if(this.state.tweetsLoaded) {
            return (
                <div>
                    <select className="custom-select" onChange={this.handleSortTweets}>
                        <option selected>Sort By</option>
                        <option value="retweet_count">Comments</option>
                        <option value="favorite_count">Likes</option>
                    </select>
                                ...
                                <div className="col-sm-9 col-md-7 col-lg-12 mx-auto">
                                    <div className="card card-signin my-3">
                                        <div className="card-body">
                                            ...
                                            <i className="fa fa-comments" aria-hidden="true">{tweet.retweet_count}</i>
                                            <i className="fa fa-thumbs-o-up" aria-hidden="true">{tweet.favorite_count}</i>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
export const ConnectedApp = AppComponent;